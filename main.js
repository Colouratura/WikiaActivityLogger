...
class Logger {
    ...
    _initWikis() {
        ...
        util.each( wikis, ( k, v ) => this._wikis.push(
            new Wiki( k, v, ( this._config.language || 'en' ), this._cookieJar, this._cache.wikis[ k ] )
        ), this );
    }
    ...
    _login( info ) {
    	...
    	io.api( info.domain, 'login', options, undefined, 'POST' )
            .then( ( function( data ) {
    		    switch( data.result ) {
    			    case 'NeedToken':
